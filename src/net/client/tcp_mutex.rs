...
impl InnerTcpConnection {
    ...
    fn insert_answer(&self, answer: Message<Bytes>) {
            ...
            Some(query) => {
                match &query.state {
                    SingleQueryState::Busy => {
                        query.state = SingleQueryState::Done(Ok(answer));
                        query.complete.notify_one();
                    }
        ...
        if query_vec.busy == 0 {
            ...
            if status.idle_timeout.is_none() {
                ...
                status.state = ConnState::Idle(Instant::now());
        ...
        for query in &mut query_vec.vec {
            match query {
                ...
                Some(q) => {
                    match q.state {
                        SingleQueryState::Busy => {
                            q.state = SingleQueryState::Done(Err(
                                arc_error.clone()
                            ));
                            q.complete.notify_one();
                        }
                        SingleQueryState::Done(_)
                        | SingleQueryState::Canceled =>
                            // Nothing to do
                ...
                match head {
                    Some(vec) => {
                        ...
                        if let Err(error) = res {
                            self.tcp_error(error);
                            let mut status = self.status.lock().unwrap();
                            status.state = ConnState::WriteError;
    ...
    fn insert(&self) -> Result<usize, &'static str> {
        ...
        for index in loc_curr..vec_len {
            match query_vec.vec[index] {
                ...
                None => {
                    Self::insert_at(&mut query_vec, index, q);
                    return Ok(index);
                }
            }
        }
    ...
    pub fn query<
        Octs: OctetsBuilder + AsMut<[u8]> + AsRef<[u8]> + Composer + Clone,
    >(
        &self,
    ...
    pub async fn get_result<Octs: Octets>(
        &self,
        ...
        let local_notify = {
            ...
            // await_holding_lock
            let mut query_vec = self.query_vec.lock().unwrap();
            query_vec.vec[index].as_mut().unwrap().complete.clone()
            // drop(query_vec);
        };
    ...
    fn cancel(&self, index: usize) {
        ...
        match &mut query_vec.vec[index] {
            ...
            Some(query) => {
                match &query.state {
                    SingleQueryState::Busy => {
                        query.state = SingleQueryState::Canceled;
                    }
                    SingleQueryState::Canceled => {
...
impl TcpConnection {
    ...
    pub fn query<
        OctsBuilder: OctetsBuilder + AsMut<[u8]> + AsRef<[u8]> + Composer + Clone,
    >(
        &self,
        query_msg: &mut MessageBuilder<
            StaticCompressor<StreamTarget<OctsBuilder>>,
        >,
    ...
    pub async fn get_result(
        ...
        match self.state {
            QueryState::Busy(index) => {
                let result =
                    self.transport.get_result(&self.query_msg, index).await;
