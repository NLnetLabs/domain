...
/// Capacity of a private channel between [InnerTcpConnection::reader] and
/// [InnerTcpConnection::run].
const READ_REPLY_CHAN_CAP: usize = 8;

/// Error reported when the TCP connection is closed and
/// [InnerTcpConnection::run] terminated.
...
#[derive(Debug)]
/// A request from [Query] to [TcpConnection::run] to start a DNS request.
...
/// Response to the DNS request sent by [InnerTcpConnection::run] to [Query].
type ChanResp = Result<Response, Arc<std::io::Error>>;

/// The actual implementation of [TcpConnection].
struct InnerTcpConnection<Octs: OctetsBuilder> {
    /// TCP connection protected by a mutex to allow read/write access by
    /// [InnerTcpConnection::run].
    stream: Futures_mutex<TcpStream>,

    /// [InnerTcpConnection::sender] and [InnerTcpConnection::receiver] are
    /// part of a single channel.
    ///
    /// Used by [Query] to send requests to [InnerTcpConnection::run].
    ...
    /// [InnerTcpConnection::run].
    /// The Option is to allow [InnerTcpConnection::run] to signal that the
...
#[derive(Clone)]
/// A single DNS over TCP connection.
pub struct TcpConnection<Octs: OctetsBuilder> {
    /// Reference counted [InnerTcpConnection].
    inner: Arc<InnerTcpConnection<Octs>>,
}
...
enum ConnState {
    ...
    /// This state represent a TCP connection that went idle and has an
    /// idle timeout.
    ///
...
/// A DNS message received to [InnerTcpConnection::reader] and sent to
/// [InnerTcpConnection::run].
// This type could be local to InnerTcpConnection, but I don't know how
