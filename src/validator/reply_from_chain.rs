...
pub struct ReplyFromChain {
    records: Arc<Mutex<Vec<Record<ParsedName<Bytes>, AllRecordData<Bytes, ParsedName<Bytes>>>>>>,
}
...
where
    CR: ComposeRequest
{
    fn send_request(&self, msg: CR) -> Box<(dyn GetResponse + std::marker::Send + Sync + 'static)> {
...
impl GetResponse for ReplyResponse {
    fn get_response(&mut self) -> Pin<Box<dyn Future<Output = Result<Message<Bytes>, Error>> + Send + Sync>> { 
	Box::pin(ready(Ok(self.response.clone())))
    }
}

fn create_reply(msg: &Message<Vec<u8>>, result: Vec<&Record<ParsedName<Bytes>, AllRecordData<Bytes, ParsedName<Bytes>>>>) -> Message<Bytes> {
    let mut target =
