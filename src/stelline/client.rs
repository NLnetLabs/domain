            ...
            match step.step_type {
                StepType::Query => {
                    ...
                    let reqmsg = entry2reqmsg(entry)
                        .map_err(StellineErrorCause::ClientError)?;
...
pub enum Client {
    // Single(Box<dyn SendRequest<AuthenticatedRequestMessage<Vec<u8>, Key>>>),
    // Multi(Box<dyn SendRequestMulti<AuthenticatedRequestMessageMulti<Vec<u8>, Key>>>),
...
fn entry2reqmsg(entry: &Entry) -> Result<RequestMessage<Vec<u8>>, Error> {
...
fn entry2msg(
    entry: &Entry,
) -> (&super::parse_stelline::Sections, Reply, Message<Vec<u8>>) {
