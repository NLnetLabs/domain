...
struct TcpReader {
    ...
    // If a process takes the lock on the reader, it should check this
    // variable and then set it to true until it is done.
    left_broken: bool,
}
...
impl Client for TcpClient {
    async fn request<'a>(
        ...
        // Therefore, if we read a message from the socket, it might not be
        // meant for us. This is why we create a oneshot channel for every
        // request sent on this stream. One of these tasks then takes the
        // lock of the reader and starts reading messages.
        //
        // If it finds a message meant for another task. It takes the channel
        // for that task and sends the message to it. It then continues
        // reading.
        //
        // If it finds its own message, it simply returns the message and
        // stops. Another task will then be able to take the lock and will
        // continue reading.
        //
...
impl TcpClient {
    async fn read_loop(&self, id: u16) -> Result<Vec<u8>, ClientError> {
        ...
        // If the lock was released by another process at a bad moment,
        // the connection is broken.
        if read.left_broken {
            return Err(ClientError::Broken);
        }

        // If this future is unexpectedly dropped from this point, it might
        // have left the reader in a broken state.
        read.left_broken = true;
        ...
        loop {
            ...
            let Some(recv_id) = buf.first_chunk::<2>() else {
                // We do not set `left_broken` to `false` to mark the
