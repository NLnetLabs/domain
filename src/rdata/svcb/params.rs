...
impl<Octs: AsRef<[u8]>> SvcParams<Octs> {
    pub fn scan<S: Scanner<Octets = Octs>>(
        ...
        // Loop over tokens. SvcbParams might be split across multiple tokens
        // if the SvcParamValue is quoted, therefore using custom
        // scan_svcb_octets scanner function.
        ...
        while scanner.continues() {
            ...
            if octs.as_ref().is_empty() {
                // If someone provides an empty params token, e.g.: SVCB 10 . ""
        ...
        for (param_key, param_value) in &key_map {
            ...
            // When the list of SvcParams is non-empty, it contains a series
...
>: SvcParamValue + Sized
{
    /// Scan a parameter value from octets from presentation format.
    ///
    /// The method should return `Ok(None)` if the type cannot parse values
    /// with `key`. It should return an error if parsing fails.
