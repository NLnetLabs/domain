...
impl KeyPair {
    ...
    where
        Octs: AsRef<[u8]>,
    {
        // The error generated by the last-attempted backend.
        //
        // NOTE: At least one backend is required to be available, so there is
        // no chance that 'error' will be returned directly. Thus, we don't
        ...
        // If no implementation worked, fail.
...
pub fn generate(
    ...
    #[cfg(feature = "ring")]
    {
        ...
        match ring::sign::generate(params, flags, &rng) {
            ...
            #[allow(unused_assignments)] // 'error' might be ignored
            Err(impl_error @ ring::GenerateError::UnsupportedAlgorithm) => {
                error = impl_error.into();
                // Try the next available implementation.
            }
            Err(error) => return Err(error.into()),
    ...
    match openssl::sign::generate(params, flags) {
        ...
        #[allow(unused_assignments)] // 'error' might be ignored
        Err(impl_error @ openssl::GenerateError::UnsupportedAlgorithm) => {
            error = impl_error.into();
            // Try the next available implementation.
        }
        Err(error) => return Err(error.into()),
...
mod tests {
    ...
    fn keypair_from_bytes() {
        for &(algorithm, _) in KEYS {
            ...
            let (sec_bytes, pub_key) = generate(&params, 257)
